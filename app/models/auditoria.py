    ...
    def obter_valor_decimal(cls, chave: str, padrao: Decimal = Decimal('0.00')) -> Decimal:
        try:
            ...
            try:
                ...
                current_app.logger.warning(f"Erro ao converter configuração '{chave}': {e}")
            except RuntimeError:
                import sys
                print(f"Erro ao converter configuração '{chave}': {e}", file=sys.stderr)
        except Exception as e:
            try:
                from flask import current_app
                current_app.logger.error(f"Erro inesperado ao obter configuração '{chave}': {e}")
            except RuntimeError:
                import sys
