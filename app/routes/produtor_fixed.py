...
def api_dashboard_produtor():
    ...
    def transacao_to_dict(t: Transacao):
        return {
            'id': t.id,
            'fatura_ref': escape(str(t.fatura_ref)) if t.fatura_ref else None,
            'status': t.status,
            'produto': escape(str(getattr(getattr(getattr(t, 'safra', None), 'produto', None), 'nome', None))),
    ...
    if venda.status == TransactionStatus.PENDENTE:
        try:
            ...
            db.session.add(Notificacao(
                ...
                mensagem=f"Pedido de {escape(venda.safra.produto.nome)} aceite! Pode proceder ao pagamento.",
            ...
            db.session.rollback()
            flash('Erro ao processar aceitacao.', 'danger')
...
def _criar_safra_from_form(produtor_id):
    ...
    return Safra(
        ...
        observacoes=escape(request.form.get('descricao', '')),
        imagem=nome_foto
    )
...
def _notificar_interessados(safra):
    ...
    for alerta in interessados:
        db.session.add(Notificacao(
            ...
            mensagem=f"Nova safra de {escape(safra.produto.nome)} em {escape(current_user.provincia.nome)}!",
...
def nova_safra():
    if not current_user.conta_validada:
        flash('A tua conta precisa de ser validada pela administracao para publicar.', 'warning')
        return redirect(url_for('produtor.dashboard'))
...
def confirmar_envio(trans_id):
    ...
    if transacao.vendedor_id != current_user.id:
        flash("Nao tens permissao para gerir esta transacao.", "danger")
        return redirect(url_for('produtor.dashboard'))

    if transacao.status != TransactionStatus.ESCROW:
        flash("Acao invalida. O pagamento deve ser confirmado primeiro.", "warning")
        return redirect(url_for('produtor.dashboard'))
    ...
    try:
        ...
        notif = Notificacao(
            ...
            mensagem=f"Encomenda {escape(transacao.fatura_ref)} enviada! Previsao: {previsao_str}",
            link=url_for('comprador.dashboard')
        )
        db.session.add(notif)

        db.session.commit()
        flash(f"Envio da fatura {escape(transacao.fatura_ref)} confirmado! O comprador foi notificado.", "success")
...
def editar_safra(id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            safra.observacoes = escape(request.form.get('descricao'))
            ...
            flash('Erro tecnico ao atualizar a safra. Verifique os valores introduzidos.', 'danger')
...
def confirmar_entrega(trans_id):
    ...
    if venda.status == TransactionStatus.ESCROW:
        ...
        db.session.add(Notificacao(
            usuario_id=venda.comprador_id,
            mensagem=f"A mercadoria {escape(venda.fatura_ref)} foi enviada. Confirme quando receber!",
            link=url_for('comprador.dashboard')
        ))
        db.session.commit()
...
def eliminar_safra(id):
    ...
    if safra.produtor_id != current_user.id:
        flash('Nao tem permissao para eliminar esta safra.', 'danger')
        return abort(403)
    ...
    if transacoes_ativas:
        flash(
            'Nao e possivel eliminar uma safra com pedidos ativos ou finalizados. Tente apenas atualizar o stock para 0.',
            'warning')
        return redirect(url_for('produtor.safras'))
