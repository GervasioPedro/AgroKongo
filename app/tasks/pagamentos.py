...
def processar_liquidacao(self, trans_id: int):
    ...
    try:
        ...
        db.session.add(LogAuditoria(
            ...
            detalhes=f"Transação {transacao.fatura_ref} liquidada: +{float(transacao.valor_liquido_vendedor):.2f} Kz ao vendedor {transacao.vendedor_id} (task {self.request.id})",
            ip="celery_task"
        ))
...
@shared_task(bind=True, base=AgroKongoTaskBase, max_retries=3, rate_limit='10/d')
...
def processar_liquidacao_sincrono(trans_id: int):
    """
    Versão síncrona para fallback quando Celery não está disponível.
    Mesma lógica da task assíncrona, mas executada diretamente.
    """
    from app.models import (
        Transacao, TransactionStatus, Notificacao,
        LogAuditoria, MovimentacaoFinanceira
    )
    from sqlalchemy.orm import joinedload
    ...
    try:
        ...
        db.session.add(LogAuditoria(
            usuario_id=None,
            acao="LIQUIDACAO_FINANCEIRA",
            detalhes=f"Transação {transacao.fatura_ref} liquidada: +{float(transacao.valor_liquido_vendedor):.2f} Kz ao vendedor {transacao.vendedor_id} (síncrono)",
            ip="sync_call"
        ))

        db.session.commit()
        current_app.logger.info(f"✅ Liquidação {transacao.fatura_ref} concluída (síncrono)")

    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Erro liquidação síncrona {trans_id}: {e}")
        raise